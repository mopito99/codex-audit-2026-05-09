...
def download_binance_vision(symbol="SOLUSDT", interval="5m", start_year=2021, end_year=2024):
    print("⏳ Iniciando descarga directa desde Binance Vision...")
    base_url = "https://data.binance.vision/data/spot/monthly/klines"
    all_dfs = []

    os.makedirs("/tmp/kline_data", exist_ok=True)

    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            file_name = f"{symbol}-{interval}-{year}-{month:02d}.zip"
            url       = f"{base_url}/{symbol}/{interval}/{file_name}"
            zip_path  = f"/tmp/kline_data/{file_name}"

            try:
                urllib.request.urlretrieve(url, zip_path)
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    csv_name = zip_ref.namelist()[0]
                    zip_ref.extract(csv_name, "/tmp/kline_data/")

                df = pd.read_csv(f"/tmp/kline_data/{csv_name}", header=None)
                all_dfs.append(df)
...
def engineer_features(df):
    if df.empty:
        ...
        return df
    print("🧠 Calculando features para el cerebro AI...")
    ...
    print(f"✅ Features listos — {df.shape[0]:,} velas × {df.shape[1]} columnas")
    return df
...
if __name__ == '__main__':
    ...
    path = '/srv/quantum_ppo/data/sol_usdt_5m.parquet'
    if not df.empty:
        df.to_parquet(path)
        print(f"💾 Parquet guardado → {path}  ({len(df):,} velas)")
