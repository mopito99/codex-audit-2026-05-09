...
// Mints del pool monitoreado. R7 migración: WETH/USDC (Gemma G87 KPI gates).
// Nombre legacy WSOL_MINT mantenido para minimizar diff — valor actual = WETH-portal mint.
// En refactor futuro renombrar a MINT_A / MINT_B.
pub const WSOL_MINT:  &str = "7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs"; // WETH-portal (8d)
pub const USDC_MINT:  &str = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"; // USDC (6d)

// Pool Orca WETH/USDC (R7). Pool anterior SOL/USDC era trampa G61.
pub const ORCA_SOL_USDC_POOL: &str = "AU971DrPyhhrpRnmEBp5pDTWL2ny7nofb5vYBjDJkR2E";
...
impl OrcaPoolInfo {
    ...
    /// Startup diagnostic: confirma precio token_a actual y direcciones de vault.
    /// R7 WETH/USDC: factor decimal = 100 (era 1000 para SOL/USDC). Rango sano: $1500-$5000.
    pub fn verify_offsets(data: &[u8]) {
        ...
        if data.len() >= 86 {
            ...
            let price = { let s = sqrt as f64 / two64; s * s * 100.0 }; // R7: WETH/USDC dec_diff=+2
            println!("  sqrt_price_x64={sqrt} liquidity={liq} tick={tick} → price_weth=${price:.2}");
            if price < 1500.0 || price > 5000.0 {
                eprintln!("  ⚠️  precio fuera de rango WETH ($1500-$5000) — verificar pool/account");
            }
        }
        if data.len() >= 245 {
