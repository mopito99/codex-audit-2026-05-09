...
const MIN_GAP_PCT: f64 = 0.0001;  // 0.1% (10 bps)
...
pub fn spawn(cache: PriceCache, paper_mode: bool) {
    tokio::spawn(async move {
        println!("[opportunity_engine] iniciado — gap_min=0.1% poll={}ms paper={}", POLL_MS, paper_mode);
        ...
        loop {
            ...
            let (orca, ray) = match (orca, ray) { (Some(a), Some(b)) => (a, b), _ => continue };
            if orca.last_update.elapsed().as_secs() > 5 || ray.last_update.elapsed().as_secs() > 5 {
                continue;
            }
            if orca.price <= 0.0 || ray.price <= 0.0 { continue; }
...

            {
                ...
                if gap > s.max_gap_seen_pct { s.max_gap_seen_pct = gap; }
            }

            if gap < MIN_GAP_PCT { continue; }

            if let Some(t) = last_alert { if t.elapsed().as_secs() < 5 { continue; } }
            last_alert = Some(Instant::now());

            println!(
                "[opportunity_engine] GAP {:.3}% | Orca={:.4} Raydium={:.4} | comprar en {} vender en {}",
                gap * 100.0, orca.price, ray.price, cheap, expensive
            );
            ...
            };
            {
                ...
                if s.gap_history.len() > HISTORY_SIZE { s.gap_history.truncate(HISTORY_SIZE); }
            }
