...
struct BundleEntry<'a> {
    ...
    #[serde(rename = "tipLamports")] tip_lamports: u64,
    #[serde(rename = "abGroup")] ab_group: &'a str,
    // Tiempo desde detección del gap hasta envío del bundle (ms) — diagnóstico latencia
    #[serde(rename = "gapToSendMs")] gap_to_send_ms: u64,
    status: &'a str,
    arb: BundleArbInfo,
}
...
impl Logger {
    ...
    pub fn record_bundle_sent(&self, bundle_id: &str, endpoint: &str, expected_profit: f64, intermediate: &str, input_usdc: f64, tip_lamports: u64, ab_group: &str, gap_to_send_ms: u64) {
        let mut inner = self.inner.lock().unwrap();
