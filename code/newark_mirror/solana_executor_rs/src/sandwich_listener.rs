...
// Pool Orca monitoreado para sandwich. R7 migración: WETH/USDC (Gemma G87 KPI gates).
// Layout +2: token_a=WETH(8d), token_b=USDC(6d). Pool anterior SOL/USDC era trampa (G61/R7).
// Cambiar a Vec<Pubkey> para multi-pool en futuro refactor.
pub const ORCA_SOL_USDC: &str = "AU971DrPyhhrpRnmEBp5pDTWL2ny7nofb5vYBjDJkR2E";
...
impl WhirlpoolState {
    ...
    /// Precio USDC por unidad de token_a. Fórmula Q64.64: (sqrt/2^64)^2 × 10^(dec_a - dec_b).
    /// R7 WETH/USDC: 10^(8-6)=100. Para SOL/USDC sería 1000. Nombre legacy `price_usdc_per_sol`
    /// retenido para minimizar diff — semántica real: USDC per token_a.
    pub fn price_usdc_per_sol(&self) -> f64 {
        let sqrt = self.sqrt_price_x64 as f64 / (1u128 << 64) as f64;
        sqrt * sqrt * 100.0 // ×10^(8-6) WETH/USDC. Era 1000 para SOL/USDC.
    }
}
        ...
        while let Ok(Some(update)) = stream.message().await {
            match update.update_oneof {
                ...
                Some(UpdateOneof::Transaction(tx_update)) => {
                    ...
                    // R7 WETH/USDC: token_a WETH=8 decimals → /1e8.
                    // (Era /1e9 cuando token_a era SOL en pool anterior).
                    let amount_usdc = if a_to_b {
                        let state_tmp = pool_state.read().unwrap()
                            .get(&pool_pubkey).cloned().unwrap_or_default();
                        amount_in as f64 / 1e8 * state_tmp.price_usdc_per_sol()
                    } else {
