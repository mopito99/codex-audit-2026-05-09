...
// Orca Whirlpool fee (0.05%)
const ORCA_FEE_PCT: f64 = 0.0005;

// ── Pool token_a config (R7 migración a WETH/USDC) ───────────────────────────
// Para SOL/USDC pool: PRICE_USD=83 (cfg.sol_price_usd), DIVISOR=1e9 (SOL 9d).
// Para WETH/USDC pool: PRICE_USD=2280 (WETH spot), DIVISOR=1e8 (WETH 8d).
// Refactor futuro: leer de .env. Hardcoded por simplicidad en R7.
const POOL_TOKEN_A_PRICE_USD: f64 = 2280.0;       // WETH ≈ $2280
const POOL_TOKEN_A_DIVISOR:   f64 = 100_000_000.0; // 10^8 (WETH decimals)
...
fn estimate_profit(
    ...
    // R7: token_a es WETH (8d, $2280) en pool WETH/USDC, no SOL.
    // sol_price_usd se mantiene en signatura para compatibilidad con tip_costs.
    let _ = sol_price_usd; // unused for victim valuation in WETH/USDC pool
    let victim_usdc = if opp.victim_a_to_b {
        opp.victim_amount_in as f64 / POOL_TOKEN_A_DIVISOR * POOL_TOKEN_A_PRICE_USD
    } else {
...
fn log_op(
    ...
    let _ = sol_price_usd; // R7: WETH price hardcoded in POOL_TOKEN_A_PRICE_USD
    let victim_usdc = if opp.victim_a_to_b {
        opp.victim_amount_in as f64 / POOL_TOKEN_A_DIVISOR * POOL_TOKEN_A_PRICE_USD
    } else {
        opp.victim_amount_in as f64 / 1e6
...
pub async fn run_sandwich_executor(
    ...
    loop {
        ...
        println!(
            ...
            victim_usdc,
            if opp.victim_a_to_b { "WETH→USDC" } else { "USDC→WETH" },
