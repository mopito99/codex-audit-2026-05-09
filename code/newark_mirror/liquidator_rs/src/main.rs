...
async fn main() -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let stats = serde_json::json!({
                    ...
                    "max_debt_cap_usd": 200.0,
                    "min_profit_usd": 2.0,
                    "max_tip_lamports": 5_000_000u64,
    ...
    // R47 C2 + R48 Q3: Telegram inbound listener with /bot_stats.
    match liquidator_rs::telegram_listener::parse_telegram_config_from_env() {
        Ok(tg_config) => {
            let listener = liquidator_rs::telegram_listener::TelegramListener::new(tg_config);
            let cb_for_tg = circuit_breaker.clone();
            let stats_for_tg = stats.clone();
            tokio::spawn(async move {
                listener.listen(cb_for_tg, stats_for_tg).await;
            });
            info!("telegram listener spawned");
        }
        Err(e) => {
            warn!(error=?e, "telegram listener disabled (config missing)");
        }
    }
